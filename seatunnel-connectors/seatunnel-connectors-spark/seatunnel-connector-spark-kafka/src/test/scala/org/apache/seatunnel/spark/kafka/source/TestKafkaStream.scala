...
class TestKafkaStream extends TestKafka with BeforeAndAfterEach {
  ...
  "KafkaStream" should "work" in {
    ...
    val data = kafkaStream.getData(environment)

    kafka.mock(testTopic, v = "row1")
    kafka.mock(testTopic, v = "row2")
    TimeUnit.MILLISECONDS.sleep(500)

    println(data)
  }

  "KafkaStream" should "fail with exception caught due to invalid config" in {
    val mConfig =
      Map("consumer." + ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG -> kafka.getBootstrapServers).asJava
